...
scenes/cards/
  card.tscn         ← visual card node
  game_card_layout.tscn
scenes/ui/
  combatant_hud.tscn
...
scripts/
  ...
  cards/
    card.gd         ← GameCard extends simple_cards.Card (drag, shine, tilt)
    card_data.gd    ← CardData Resource (id, name, cost, type, damage, level, etc.)
    game_card_layout.gd
  ui/
...
data/cards/
  slash.tres        ← Cost 3, dmg 6, ATTACK
  heavy_slash.tres  ← Cost 5, dmg 10, ATTACK
  focus.tres        ← Cost 0, energy +4, SKILL
  move_up/down/left/right.tres ← free MOVE cards
```
...
- Shows 3 shuffled cards from `REWARD_POOL` (currently slash/heavy_slash/focus)
...
- Reward pool expansion beyond 3 starter cards
